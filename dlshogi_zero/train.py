...
training_database_path = args.training_database
...
def mini_batch(database, window_size, batch_size):
    ...
    for i, (hcps, repetition, total_move_count, legal_moves, visits, game_result) in enumerate(database.get_training_batch(window_size=window_size, batch_size=batch_size)):
        # input features
        for j, hcp in enumerate(hcps):
            board.set_hcp(hcp)
            encode_position(board, j, features[i])
            if j == 0:
                color = board.turn
