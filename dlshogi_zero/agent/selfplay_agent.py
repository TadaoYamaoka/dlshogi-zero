...
class SelfPlayAgentGroup:
    def __init__(self, policy_value_batch_maxsize):
        ...
        self.agents = [SelfPlayAgent()] * policy_value_batch_maxsize

    def selfplay():
        # 探索経路のバッチ
        trajectories_batch = [TrajectorEntry()] * self.policy_value_batch_maxsize

        # 全スレッドが生成した局面数が生成局面数以上になったら終了
        while made_teacher_nodes < teacher_nodes and stopflg:
            # すべての対局についてシミュレーションを行う
            for i in range(policy_value_batch_maxsize):
                agents[i].playout(trajectories_batch[i])
        pass
...
class SelfPlayAgent:
    ...
    def init_model(self):
        self.model = ResNet(res_blocks=10, filters=192)
        self.model.compile(optimizer=tf.train.MomentumOptimizer(learning_rate=0.01, momentum=0.9),
